...
use quote::quote;
use syn::{
    File, FnArg, ForeignItem, GenericArgument, Ident, ImplItem, Item, ItemStruct, PathArguments,
    ReturnType, Type,
};
...
pub fn generate(
    ...
    blocklist: &mut HashSet<String>,
    versions: &HashMap<String, (u32, u32, u32)>,
) {
    // Load the input file
    ...
    module.push_str("use crate::foundation::VersionT;\n\n");
    ...
    generate_apis(&file, &mut module, versions);
    generate_define_macros(target_headers, &mut module, versions);
...
fn generate_apis(file: &File, module: &mut String, versions: &HashMap<String, (u32, u32, u32)>) {
    for item in &file.items {
        if let Item::Struct(item) = item {
            ...
            generate_api(module, item, versions);
        }
    }
}

fn generate_api(src: &mut String, item: &ItemStruct, versions: &HashMap<String, (u32, u32, u32)>) {
    ...
    let version = versions.get(&api_name);
    if let Some(version) = version {
        ...
        let reg_impl = quote! {
            impl crate::Api for #struct_name {
                ...
                const VERSION: VersionT = VersionT { major: #major, minor: #minor, patch: #patch };
            }
        };
...
fn generate_define_macros(
    ...
    // Versions were pre-parsed during bindings generation
    for (name, (major, minor, patch)) in versions {
        let name = format!("{}_VERSION", name.to_shouty_snake_case());
        let ident = Ident::new(&name, Span::call_site());
        let constant = quote! {
            pub const #ident: VersionT = VersionT { major: #major, minor: #minor, patch: #patch };
        };
        module.push_str(&constant.to_string());
    }
}
